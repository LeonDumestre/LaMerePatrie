...
import { getServerDataInd, getServerStats, getMemberStatsInd, getServersData } from "../Tools/global.js";

const cooldowns = new Map();
const cooldownsHorloge = new Map();

export const name = "goulag";
export const description = "Permet de timeout une personne pour une durée max de 5 minutes";
...
export async function run(bot, message, args) {
	...
	// Envoi au goulag
	try {
		if (goulagRole && !targetMember.permissions.has(PermissionFlagsBits.Administrator) && !targetMember.roles.cache.some(r => r == goulagRole)) await targetMember.roles.add(goulagRole);
	}
	...
	setTimeout(() => {
		...
		embed.setColor(3708728).setDescription(msg).setFooter({ text: " " });
