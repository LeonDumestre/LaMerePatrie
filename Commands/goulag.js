...
//La commande n'est pas lancé
    //Si le goulagueur est au goulag
    ...
    //Crée un membre dans data.json s'il n'existe pas
        ...
        let nbrMembers = stats.membres.length
        ...
        if (!targetMemberExist || !authorMemberExist) {
            ...
            if (!targetMemberExist) {
                ...
                let newtargetMember = {"id":targetMember.id,"winBingo":0,"goulag":{"nbrFoisAllé":0,"nbrFoisEnvoyé":0,"tpsPassé":0,"nbrEvent":{"auteurEnvoyé":0,"tpsDoublé":0,"tpsDivisé":0},"ennemis":[],"cibles":[]},"dateQuit":0}
            ...
            if (!authorMemberExist && targetMember.id != authorMember.id) {
                ...
                let newMembre2 = {"id":authorMember.id,"winBingo":0,"goulag":{"nbrFoisAllé":0,"nbrFoisEnvoyé":0,"tpsPassé":0,"nbrEvent":{"auteurEnvoyé":0,"tpsDoublé":0,"tpsDivisé":0},"ennemis":[],"cibles":[]},"dateQuit":0}
    ...
    //Choix random et envoi du message d'envoi au goulag
        ...
        //envoie du message d'envoie et gestion de l'horologe du footer
        ...
        let msgEnvoie = await message.reply({ embeds: [embed] })
    ...
    //Envoi au goulag
        ...
        if (!ennemiExiste) {
            let ennemi = {"id":authorMember.id,"nbr":1}
        ...
        if (!cibleExiste) {
            let cible = {"id":targetMember.id,"nbr":1}
            stats.membres[indAuthorMember].goulag.cibles.push(cible)
        }
