	...
	async run(bot, message, args) {
        ...
        if (args.getString('paramètre') && args.getChannel('salon') && args.getChannel('salon').type === 0) {
            if (args.getString('paramètre') === 'add') {
                if (!serversData.servers[serverDataInd].cmdChannels.find(c => c.id === args.getChannel("salon").id)) 
                    serversData.servers[serverDataInd].cmdChannels.push(args.getChannel('salon').id);
            }
