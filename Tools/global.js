...
export function getServersData() {
	// check if exists servers.json exists
	let serversDataExist = false;
	fs.readdirSync("./ServersData").filter(f => f.endsWith(".json")).forEach(async file => {
		if (file.split(".json").join("") == "servers") serversDataExist = true;
	});
	if (!serversDataExist) this.generateServersDataFile();

	const file = fs.readFileSync("./ServersData/servers.json");
	return JSON.parse(file);
}

export function generateServersDataFile() {
	const serversData = { "servers": [] };
	fs.writeFileSync("./ServersData/servers.json", JSON.stringify(serversData));
}

export function getServerStats(guildId) {
...
export function getServerDataInd(guildId) {
	const link = "./ServersData/servers.json";
	const serversData = module.exports.getServersData();
...
export function getMemberStatsInd(guildId, member) {
	let statsExist = false;
	fs.readdirSync("./ServersData/Stats").filter(f => f.endsWith(".json")).forEach(async file => {
		if (file.split(".json").join("") == guildId) statsExist = true;
	});
	...
	const serverStats = JSON.parse(file);
